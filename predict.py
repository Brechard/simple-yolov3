...
from src import constants, helpers, visualize
from src.data.dataset import Dataset
from src.yolo.yolov3 import YOLOv3

FLAGS = flags.FLAGS
# Strings
flags.DEFINE_string("db", 'COCO', "Dataset name. Used for translating from label id to label text.")
...
def detect(model, dataset_name: str, image_path: str, title: str = "", output_path: str = None):
    ...
    helpers.dir_exists('/'.join(output_path.split("/")[:-1]))
...
def main(_argv):
    ...
    if FLAGS.weights_path:
        model.load_models(Dataset(FLAGS.db), for_training=False)
    ...
    detect(model, FLAGS.db, FLAGS.img_path, FLAGS.title, FLAGS.output_path)
